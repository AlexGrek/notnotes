...
fun Application.module() {
    ...
    install(StatusPages) {
        ...
        exception<NoSuchElementException> { call, cause ->
            call.respond(HttpStatusCode.NotFound, mapOf("error" to (cause.toString())))
        }
        // Handle cases where the client provides invalid input (e.g., missing parameter)
        exception<IllegalArgumentException> { call, cause ->
            call.respond(HttpStatusCode.BadRequest, mapOf("error" to (cause.toString())))
        }
        // Catch-all for any other unexpected errors
        exception<Throwable> { call, cause ->
            // In a real production app, you would also log the 'cause' here
            call.respond(HttpStatusCode.InternalServerError, mapOf("error" to (cause.toString())))
        }
    }
